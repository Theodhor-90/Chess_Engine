...
- Single pawn push (one square forward, blocked by any piece)
...
- Pawn captures (diagonal, one square, only onto opponent pieces)
- En passant captures when `Position::en_passant()` returns a target square, encoded with `MoveFlag::EN_PASSANT`
- Promotions on the 8th/1st rank generating four separate `Move` entries per promotion square (knight, bishop, rook, queen), using the appropriate `MoveFlag` promotion and promotion-capture flags
...
- Knight move generation function that iterates over friendly knights, looks up the attack table, removes friendly-occupied squares, and produces `Move` values (quiet moves and captures with appropriate flags)
...
- Attack table initialization (compile-time `const` or runtime-once initialization)
- King move generation function that looks up the attack table for the king's square, removes friendly-occupied squares, removes squares attacked by the opponent (using `Position::is_square_attacked`), and produces `Move` values (quiet moves and captures with appropriate flags)
- Unit tests verifying king moves exclude friendly-occupied squares and opponent-attacked squares

## Exit Criteria

1. Pawn move generation correctly produces all pseudo-legal pawn moves — single push, double push, captures, en passant, and all four promotion types — for both white and black.
...
- **Milestone M01 complete** — requires the `types` crate (`Bitboard`, `Square`, `Piece`, `Color`, `Move`, `MoveFlag`, `File`, `Rank`, `PieceKind`) and the `board` crate (`Position`, `Position::piece_bitboard`, `Position::occupied`, `Position::occupied_by`, `Position::side_to_move`, `Position::en_passant`, `Position::is_square_attacked`).

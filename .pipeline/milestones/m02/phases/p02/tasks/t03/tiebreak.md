...
| 2 | **Modify** | `crates/movegen/src/lib.rs` | Add `pub mod queens;` declaration; add re-exports for `generate_queen_moves` and `generate_sliding_moves` |
...
- **Assert**: `moves.len() == 27` — only the queen is on the board (no bishops or rooks), so the combined function produces the same 27 moves.

Tests 1, 2, and 3 correspond to the three FEN positions explicitly required by the task spec. Tests 4 and 5 verify basic correctness of the queen-as-bishop+rook union and the combined interface.

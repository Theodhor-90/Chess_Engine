...
4. Iterate over each set bit in the queen bitboard (each queen square):
   - Construct `Square` from the bit index via `Square::new(from_idx as u8).expect("valid square")` (allowed in iteration where indices come from valid bitboard bits — though note: project convention says no `unwrap()` in library code. The sibling files `bishops.rs` and `rooks.rs` both use `.expect("valid square")` in this exact pattern, so follow that established convention).
...
Three tests for queen generation matching the three specified positions, plus a test for `generate_sliding_moves`:
...
2. **Midgame open board** (`r1bqk2r/ppppbppp/2n2n2/4p3/2B1P3/5N2/PPPP1PPP/RNBQ1RK1 b kq - 5 4`): Black to move. Black queen on d8 — compute expected moves by analyzing the board. The black queen on d8 has bishop-like diagonals and rook-like ranks/files. Blocked by own pieces on c7, d7, e7. Available squares: check all rays from d8 given occupancy. This test verifies a non-trivial midgame position.

3. **Sliding pieces blocked by pawns** (`4k3/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQ - 0 1`): White to move. White queen on d1 is blocked by pawns on c2, d2, e2. Expected: 0 queen moves.

4. **`generate_sliding_moves` test**: Use the empty-board position with a white queen on d4 (`4k3/8/8/8/3Q4/8/8/4K3 w - - 0 1`). Verify that `generate_sliding_moves` produces 27 moves (queen has 27 squares from d4 on an empty board: 13 diagonal + 14 orthogonal). Also verify it matches the count from calling all three generators individually.
...
- **Action**: Call `generate_queen_moves(&pos, &mut moves)`
- **Assert**: Verify the exact count of queen moves and that the generated moves are correct for the black queen on d8 given the occupancy. The queen's bishop-like rays from d8 are blocked by own pieces on c7 and e7. The rook-like rays: rank-8 blocked by pieces on c8 and e7(bishop); file-d blocked by d7 pawn. The only possible squares need careful analysis. Assert the exact move count.

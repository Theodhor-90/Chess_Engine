Now I have a clear picture. Let me synthesize the definitive spec.

Recurring feedback issues across all three iterations:
1. **LLM preamble text** — all three versions included stray reasoning text before the heading (flagged every time)
2. **Vague "etc."** — v1 used "etc." in test positions; v2 and v3 fixed this with specific CPW positions
3. **Exit criterion #5** — v3 feedback correctly identified that restating global CI conventions is redundant; should be removed

The spec content itself converged well by v2/v3 — it's well-scoped, well-aligned with the master plan, and appropriately specific. The definitive version removes the preamble, keeps the specific CPW positions, and drops the redundant CI exit criterion.

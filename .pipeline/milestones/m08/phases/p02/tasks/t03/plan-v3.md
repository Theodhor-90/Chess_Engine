...
10. **SPRT-tune aspiration window delta**: Test candidate ASPIRATION_DELTA values, accept SPRT winner, update the constant
11. **SPRT-tune aspiration window widen factor**: Test candidate ASPIRATION_WIDEN_FACTOR values, accept SPRT winner, update the constant
12. **Update unit tests**: Modify `futility_margins_match_spec` and `lmr_table_values_correct` to assert the new tuned values
13. **Final verification**: `cargo fmt --check && cargo clippy -- -D warnings && cargo test`

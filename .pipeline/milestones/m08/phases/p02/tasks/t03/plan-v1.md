...
- `log_likelihood_ratio(wins: u32, draws: u32, losses: u32, elo0: f64, elo1: f64)` → `f64`: Compute the SPRT LLR using the trinomial model. Given W/D/L counts, compute the log-likelihood ratio for H1 (Elo diff = elo1) vs H0 (Elo diff = elo0).
...
The LLR for each game pair uses the BayesElo/pentanomial model simplified to the trinomial (W/D/L) model:
1. Convert `elo0` and `elo1` to expected scores `s0` and `s1` via `elo_to_score()`
2. Given observed proportions `w = W/N`, `d = D/N`, `l = L/N` (where `N = W+D+L`):
   - Under H0: expected proportions derived from `s0` (model: `w0 = s0^2`, `l0 = (1-s0)^2`, `d0 = 2*s0*(1-s0)`) — the BayesElo trinomial model
   - Under H1: expected proportions derived from `s1` similarly
3. `LLR = N * (w * ln(w1/w0) + d * ln(d1/d0) + l * ln(l1/l0))` — using model probabilities, not observed proportions, in the ratio
4. Actually, the standard simplified approach: `LLR = 0.5 * N * (s1 - s0) * (2 * observed_score - s0 - s1) / variance` where `observed_score = (W + D/2) / N` and `variance = ((W*(1-observed_score)^2 + D*(0.5-observed_score)^2 + L*(0-observed_score)^2) / N)`. This is the Gaussian approximation used by most chess SPRT implementations (fishtest, cutechess).

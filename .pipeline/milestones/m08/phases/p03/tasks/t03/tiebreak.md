...
- **CuteChess-CLI**: Built from source during CI setup. The CuteChess repository (`https://github.com/cutechess/cutechess`) is cloned at tag `v1.3.1`, built with CMake (Qt5 dependency satisfied by `qtbase5-dev`), producing the `cutechess-cli` binary. Pre-built Linux binaries are not reliably available from the releases page, so building from source is the robust approach. The runner is pinned to `ubuntu-22.04` to ensure `qtbase5-dev` is available — see Deviation 3 for rationale.
- **Rust toolchain**: The `dtolnay/rust-toolchain@stable` action to install the stable Rust toolchain in CI.
- **GitHub Actions runner**: `ubuntu-22.04` provides the execution environment.
- **Existing project artifacts**: The `tools/selfplay/openings.txt` file (EPD format opening positions created in P01/T03). This file is committed to the repository and is available after the `actions/checkout` step — no additional download is needed.
...
A single job `regression-test` running on `ubuntu-22.04` with these steps:
...
8. **Run 100-game head-to-head tournament** — Invoke CuteChess-CLI directly:
   ...
   **Time control justification**: `tc=1+0.01` (1 second base + 10ms increment) keeps the total 100-game tournament under 5 minutes in CI. The engines are near-identical builds playing head-to-head, so ultra-fast time control is sufficient — we are detecting regressions, not measuring absolute strength. Using `-rounds 50 -games 2 -repeat` plays 100 total games (2 games per round, colors swapped each pair, from 50 openings).

   **Openings file**: The file `tools/selfplay/openings.txt` is committed to the repository (created in P01/T03) and is available after the `actions/checkout` step. No additional download is needed.
...
9. **Parse results and detect regression** — An inline shell script that parses the CuteChess-CLI output and applies the regression threshold. The complete script:
   ...
   awk -v w="$W" -v d="$D" -v l="$L" 'BEGIN {
     ...
     # awk log() is natural log; log10(x) = log(x)/log(10)
     # Positive Elo = PR is stronger; negative Elo = PR is weaker
     elo = -400.0 * log(1.0/score - 1.0) / log(10.0)

     # Standard error of the score proportion
     se = sqrt(score * (1.0 - score) / n)

     # Lower bound of 95% CI (pessimistic for PR engine)
     ...
     # Upper bound of 95% CI (optimistic for PR engine)
     ...
     # Regression check: we flag a regression if we have 95% confidence
     # that the true Elo is below -30. This requires the UPPER bound
     # (the most optimistic estimate) of the 95% CI to be below -30.
     # If even the best-case Elo estimate is worse than -30, the
     # regression is statistically significant at p<0.05.
     if (elo_upper < -30.0) {
       printf "REGRESSION DETECTED: Elo upper bound (%+.1f) < -30\n", elo_upper
       printf "Even the optimistic 95%% CI bound is below -30 Elo.\n"
       exit 1
     } else {
       printf "NO REGRESSION: Elo upper bound (%+.1f) >= -30\n", elo_upper
       exit 0
     }
   }'
   ```

   **Statistical interpretation**: To claim "Elo drops by more than 30 at p<0.05" we need 95% confidence that the true Elo difference is below -30. This means the *entire* 95% CI must lie below -30. Since `elo_upper` is the optimistic (highest) end of the CI, checking `elo_upper < -30` is the correct test: if even the upper bound of the CI is below -30, then we are 95% confident the true Elo is below -30.

   **Sign convention**: The PR engine is the first engine passed to CuteChess-CLI. CuteChess-CLI reports `W - L - D` from the first engine's perspective. So `W` = PR wins, `L` = PR losses. A score < 0.5 means the PR engine is weaker → negative Elo → regression. The formula `-400 * log10(1/score - 1)` yields negative Elo when score < 0.5 and positive Elo when score > 0.5. `score_upper = score + 1.96*SE` maps to `elo_upper` (the optimistic end for the PR engine), and `score_lower = score - 1.96*SE` maps to `elo_lower` (the pessimistic end).

   **Edge case handling**: If score = 0 (all losses), the log formula is undefined; the script detects this before entering awk and immediately flags regression. If score = 1 (all wins), the script immediately passes. For extreme-but-not-zero scores, `score_lower` and `score_upper` are clamped to `[0.001, 0.999]` to avoid log(0).
...
**Test 3 — CuteChess-CLI builds from source in CI**: The build dependencies install on `ubuntu-22.04`, the repository clones at v1.3.1, and CMake produces a working binary. Verification: `./cutechess-cli --version` outputs version information.
...
- `W=50, L=50, D=0` → score=0.5, Elo≈0, elo_upper≈+69 → should pass (elo_upper >= -30)
- `W=10, L=90, D=0` → score=0.1, Elo≈-191, elo_upper≈-120 → should flag regression (elo_upper < -30)
- `W=0, L=100, D=0` → all losses, immediate regression flag
- `W=100, L=0, D=0` → all wins, immediate pass
- `W=40, L=60, D=0` → score=0.4, Elo≈-70, elo_upper≈+1 → should pass (elo_upper >= -30, CI too wide to confirm regression)
- `W=25, L=75, D=0` → score=0.25, Elo≈-191, elo_upper≈-95 → should flag regression
- `W=20, D=60, L=20` → score=0.5, Elo≈0, elo_upper well above -30 → should pass
...
1. **Create `.github/workflows/` directory** — Ensure the directory structure exists (no workflows exist in the repository yet).
...
2. **Write `.github/workflows/regression.yml`** — The complete workflow file containing:
   ...
   - Single job `regression-test` on `ubuntu-22.04` with the 9 steps described in Section 3
   - The inline regression detection script from Section 3, step 9 (embedded as a `run:` block in the final step)
   - A comment on the `runs-on` line explaining the `ubuntu-22.04` pin (Qt5 dependency)
...
# Verify the runner is pinned to ubuntu-22.04
grep 'ubuntu-22.04' .github/workflows/regression.yml
...
# Verify the openings file exists in the repo
ls -la tools/selfplay/openings.txt
...
# Test: equal score (should pass — elo_upper well above -30)
echo "Testing W=50 D=0 L=50 (Elo≈0, should pass)..."
awk -v w=50 -v d=0 -v l=50 'BEGIN {
  n=w+d+l; score=(w+d*0.5)/n
  elo=-400.0*log(1.0/score-1.0)/log(10.0)
  se=sqrt(score*(1.0-score)/n)
  su=score+1.96*se; if(su<=0.001)su=0.001; if(su>=0.999)su=0.999
  elo_upper=-400.0*log(1.0/su-1.0)/log(10.0)
  printf "score=%.3f elo=%+.1f elo_upper=%+.1f -> %s\n", score, elo, elo_upper, (elo_upper<-30)?"FAIL":"PASS"
}'

# Test: heavy losses (should flag regression — elo_upper still far below -30)
...
# Test: moderate loss (should pass — CI too wide at 100 games to confirm regression)
echo "Testing W=40 D=0 L=60 (Elo≈-70, should pass because elo_upper > -30)..."
awk -v w=40 -v d=0 -v l=60 'BEGIN {
  n=w+d+l; score=(w+d*0.5)/n
  elo=-400.0*log(1.0/score-1.0)/log(10.0)
  se=sqrt(score*(1.0-score)/n)
  su=score+1.96*se; if(su<=0.001)su=0.001; if(su>=0.999)su=0.999
...
# Test: severe loss (should flag regression — even optimistic bound below -30)
echo "Testing W=25 D=0 L=75 (Elo≈-191, should flag regression)..."
awk -v w=25 -v d=0 -v l=75 'BEGIN {
  n=w+d+l; score=(w+d*0.5)/n
  elo=-400.0*log(1.0/score-1.0)/log(10.0)
  se=sqrt(score*(1.0-score)/n)
  su=score+1.96*se; if(su<=0.001)su=0.001; if(su>=0.999)su=0.999
...
# Test: draws-heavy equal result (should pass)
echo "Testing W=20 D=60 L=20 (Elo≈0, should pass)..."
awk -v w=20 -v d=60 -v l=20 'BEGIN {
  n=w+d+l; score=(w+d*0.5)/n
  elo=-400.0*log(1.0/score-1.0)/log(10.0)
  se=sqrt(score*(1.0-score)/n)
  su=score+1.96*se; if(su<=0.001)su=0.001; if(su>=0.999)su=0.999
...
- **Chosen approach**: An inline awk script computes the Elo difference and its 95% confidence interval from the 100-game W/D/L results using the logistic Elo formula. It flags a regression if and only if the upper bound of the 95% CI is below -30 Elo — meaning even the most optimistic estimate of the PR engine's strength is worse than -30 Elo, establishing p<0.05 confidence that the true Elo is below -30. The delta-method approximation (applying the nonlinear Elo conversion to `score ± 1.96*SE`) is acknowledged as an approximation that is adequate for n=100, where the confidence interval is inherently wide.

**Deviation 3: Pinning CI runner to `ubuntu-22.04` instead of `ubuntu-latest`**

- **Parent spec states**: No specific runner version requirement.
- **Why pinning is necessary**: CuteChess-CLI v1.3.1 requires Qt5 (`qtbase5-dev`). The `ubuntu-latest` runner alias currently resolves to Ubuntu 22.04, where `qtbase5-dev` is available. However, `ubuntu-latest` will eventually migrate to Ubuntu 24.04+ where Qt5 packages may be deprecated or unavailable in favor of Qt6. Pinning to `ubuntu-22.04` ensures the Qt5 dependency is reliably satisfied without introducing a CI break when GitHub updates the `ubuntu-latest` alias.
- **Chosen approach**: Pin the runner to `ubuntu-22.04`. A comment in the workflow file explains the pin so future maintainers know to update if CuteChess-CLI adds Qt6 support or if a pre-built binary becomes available.
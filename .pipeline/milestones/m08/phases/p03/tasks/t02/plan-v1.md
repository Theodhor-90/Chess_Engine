No tests in the existing SPRT code. Now I have everything I need. Let me produce the plan.
...
- `fn parse_args() -> Args` — Manual arg parsing (same pattern as T01's `tournament` tool). All SPRT-specific parameters (`--elo0`, `--elo1`, `--alpha`, `--beta`) are required with defaults as specified.

- `fn print_usage()` — Print usage help to stderr.

- `fn run_batch(cutechess: &str, baseline: &str, candidate: &str, games: u32, tc: &str, openings: Option<&str>, concurrency: u32) -> (u32, u32, u32)` — Invoke CuteChess-CLI as a subprocess with:
  ...
  Capture stdout, parse the final "Score of ..." line using `parse_score_line()`, return (wins, losses, draws) from the perspective of Candidate (first engine listed in the score line).

- `fn parse_score_line(line: &str) -> Option<(u32, u32, u32)>` — Parse CuteChess-CLI score output format `"Score of X vs Y: W - L - D [pct]"`. Returns `(W, L, D)`. Same implementation as the tournament tool.

- `fn elo_to_score(elo: f64) -> f64` — Convert an Elo difference to an expected score: `1.0 / (1.0 + 10^(-elo/400))`.

- `fn log_likelihood_ratio(wins: u32, draws: u32, losses: u32, elo0: f64, elo1: f64) -> f64` — Compute the log-likelihood ratio using the trinomial model:
...
while total_games < max_games:
    ...
    if llr >= upper: print "ACCEPT: H1 (candidate is stronger)"; exit
    if llr <= lower: print "REJECT: H0 (no significant difference)"; exit
print "INCONCLUSIVE: max games reached"
```
...
2. **Rewrite `tools/sprt/src/main.rs`** — Replace the existing direct-engine-driving implementation with the CuteChess-CLI wrapper approach:
   ...
   - `run_batch()` function (new — invokes CuteChess-CLI subprocess, parses output)

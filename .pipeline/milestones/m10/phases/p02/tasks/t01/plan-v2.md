Now I have everything needed. Here is the revised plan:
...
- `fn play_game(opening_fen: &str, node_limit: u64, max_moves: u32) -> (Vec<TrainingRecord>, Option<GameResult>)` — plays a single self-play game:
  ...
  5. After obtaining the best move from `search()`, record a `TrainingRecord` with `pos.to_fen()`, the captured search score, and a placeholder result of `0.0`

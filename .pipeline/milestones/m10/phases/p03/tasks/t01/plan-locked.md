Now I have all the context. The feedback identifies two correctness issues, both about the same root problem: the `push_move` method signature on line 59 of the plan omits the `captured: Option<Piece>` parameter, while other sections correctly include it. I need to fix the signature in the `AccumulatorStack` methods description.
...
- `push_move(&mut self, pos: &Position, mv: Move, captured: Option<Piece>, network: &Network)` — called after `pos.make_move(mv)`. Receives `captured` from `UndoInfo.captured` since the captured piece is no longer on the board. Clones the current top, then calls `compute_and_apply_deltas` to determine and apply feature deltas for the move. For king moves, marks `king_moved` and defers full refresh. For non-king moves, computes add/remove deltas based on the move type and applies them incrementally.
...
`fn compute_and_apply_deltas(state: &mut AccumulatorState, pos: &Position, mv: Move, captured: Option<Piece>, network: &Network)`

This function receives `captured` from the caller (`push_move`) and determines what features changed due to the move:
...
2. **Captured piece** (regular capture): Uses the `captured: Option<Piece>` parameter. Remove feature for (king_sq, to_sq, captured_kind, captured_color) for each perspective.
...
**`negamax` and `quiescence` modification**: At every `pos.make_move(mv)` call, add `ctx.acc_stack.push_move(pos, mv, undo.captured, network)` immediately after. At every `pos.unmake_move(mv, undo)`, add `ctx.acc_stack.pop()` immediately before.
...
Note: `push_move` takes `undo.captured` as a parameter to know what piece was captured, since the position after `make_move` no longer has the captured piece on the board. This is consistent across the `push_move` method signature, `compute_and_apply_deltas` helper, and all call sites.
...
2. **`crates/nnue/src/accumulator.rs`** — Define `AccumulatorStack` struct with `push_move(&mut self, pos: &Position, mv: Move, captured: Option<Piece>, network: &Network)`, `pop`, `refresh_if_needed`, and `evaluate` methods. Implement `compute_and_apply_deltas(state, pos, mv, captured, network)` for all move types (quiet, capture, en passant, promotion, castling, king moves).
...
- **Alternative chosen**: The accumulator stack is managed at the search layer (`crates/search`), which already depends on both `crates/board` and `crates/nnue`. The `push_move` call is placed immediately after `pos.make_move(mv)` in the search code, receiving the captured piece from `UndoInfo.captured`. The full data flow is: `push_move(&mut self, pos, mv, captured, network)` forwards `captured` to `compute_and_apply_deltas(state, pos, mv, captured, network)`, which uses it to determine which features to remove. This preserves the clean crate dependency graph while achieving the same incremental update behavior.
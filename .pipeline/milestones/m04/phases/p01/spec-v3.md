Good. The search crate already checks time every 1024 nodes (`ctx.nodes & 1023 == 0`). The current `SearchContext` uses an `aborted: bool` field, not an `AtomicBool`. This phase needs to extend that mechanism to also check an external atomic stop flag. Now I have all the information to produce the revised spec.
...
3. **Atomic stop flag**: The engine main loop creates a `Arc<AtomicBool>` stop flag shared between the main thread and the search thread. The main thread sets the flag to `true` upon receiving `stop` or `quit`. The search crate's existing `SearchContext` already checks for abort conditions every 1024 nodes (`ctx.nodes & 1023 == 0` in `check_time`). This phase extends `SearchContext` to accept an external `Arc<AtomicBool>` and check it at the same 1024-node interval alongside the existing time check — this phase does not change the check frequency, it only adds the stop flag as an additional abort source within the existing check cadence.

4. **Search thread model**: The `go` command spawns a search thread. Only one search can be active at a time. The main loop remains responsive to `stop` and `quit` while search runs. When search completes or is stopped, the engine emits `bestmove`.
...
Deliverables: `crates/uci/` crate with `UciCommand` enum, `GoParams` struct, and `parse` function. Unit tests for all command variants including edge cases.

**T02 — UCI output**

Format and emit `id` (name, author), `uciok`, `readyok`, `bestmove`, and `info` lines (with depth, score, nodes, nps, pv, time fields) to stdout. Output conforms to the UCI specification so standard GUIs can parse it.
...
This task extends the search crate's `SearchContext` to accept an `Arc<AtomicBool>` stop flag. The existing `check_time` method (called every 1024 nodes) is extended to also check the stop flag and set `aborted = true` if the flag is set. No change to the check frequency or node-count interval.
...
5. All unit tests in `crates/uci/` and integration tests in `crates/engine/` pass

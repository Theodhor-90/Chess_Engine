Good — `Move` has a `Display` implementation producing UCI coordinate notation (e.g., `e2e4`, `a7a8q`). Now I have everything I need.
...
3. **Atomic stop flag**: The engine main loop shares an `AtomicBool` stop flag between the main thread and the search thread. The search thread checks this flag periodically (at the same node-count interval the search already uses for time checks). The main thread sets the flag upon receiving `stop` or `quit`.
...
5. All unit tests pass; `cargo clippy -- -D warnings` and `cargo fmt --check` are clean

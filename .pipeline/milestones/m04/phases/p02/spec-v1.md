...
5. **Default `movestogo` estimate**: When `movestogo` is not provided by the GUI (sudden-death time control), the engine uses a reasonable default estimate (e.g., 25 moves remaining) to avoid spending too much time early or flagging late.
...
**Deliverables**: A time allocation function (or small module) that, given `wtime`, `btime`, `winc`, `binc`, `movestogo`, and the side to move, returns a `Duration` representing the time budget for the current move. The formula is `time_left / moves_to_go + increment`, with a sensible default for `movestogo` when it is absent. This replaces the hardcoded time budget logic currently in `crates/engine/src/main.rs`. Unit tests verify correct budgets for various clock scenarios (with/without increment, with/without `movestogo`, low time, etc.).
...
**Deliverables**: Full support for all `go` sub-commands: `depth` (search to a fixed depth and stop), `nodes` (search a fixed number of nodes and stop), `movetime` (search for exactly the given milliseconds), `infinite` (search until `stop` is received), and `ponder` (search on the opponent's time; respond to `ponderhit` by switching to normal time management, respond to `stop` by returning `bestmove`). The `GoParams` struct fields already parsed in P01 are now acted upon. The `stop` command always causes the engine to emit `bestmove`. Tests verify each variant produces correct behavior.

## Exit Criteria

1. Time allocation correctly computes a per-move budget from `wtime`, `btime`, `winc`, `binc`, and `movestogo` (with a default when `movestogo` is absent)
...
4. `go nodes N` searches approximately N nodes and returns `bestmove`
5. `go movetime N` searches for approximately N milliseconds and returns `bestmove`
...
- New or extended time allocation module/function (in `crates/uci/` or `crates/search/`)
...
- Unit tests for time allocation across various clock scenarios

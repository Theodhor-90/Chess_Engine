...
fn chebyshev_distance_to_center(sq_idx: u32) -> i32 {
    ...
    let min_dist = [
        (file - 3).abs().max((rank - 3).abs()),  // d4
        (file - 3).abs().max((rank - 4).abs()),  // d5
        (file - 4).abs().max((rank - 3).abs()),  // e4
        (file - 4).abs().max((rank - 4).abs()),  // e5
...
1. **Create `crates/eval/src/king_centralization.rs`**:
   ...
   - Implement `chebyshev_distance_to_center(sq_idx: u32) -> i32`

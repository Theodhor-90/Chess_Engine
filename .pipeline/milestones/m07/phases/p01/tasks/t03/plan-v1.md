Good. Now I have a complete understanding. Let me produce the implementation plan.

## 1. Deliverables

- **Modify**: `crates/eval/src/pawn.rs` — Add passed pawn king proximity bonus function, blockade penalty function, and exponential advancement bonus constants. Add unit tests for advancement rank scaling, blockade penalty, and king proximity scoring.
...
- **Modify**: `crates/search/src/lib.rs` — Add a `PawnHashTable` field to `SearchContext`, initialize it in `search()`, and pass `&mut ctx.pawn_table` to `chess_eval::evaluate()` at both call sites (line 128 in `quiescence` and line 329 in `negamax`). Update test helper `test_ctx()` and affected tests.
...
**New constants**:
- `PASSED_PAWN_ADVANCE_BONUS_MG: [i32; 8]` and `PASSED_PAWN_ADVANCE_BONUS_EG: [i32; 8]` — exponentially scaling bonuses by rank index (0 for ranks 1/8, increasing non-linearly toward promotion rank). These replace or extend the existing `PASSED_PAWN_BONUS_MG/EG` in the context of the full passed pawn evaluation. Since the basic advancement bonus is already in `evaluate_color()` from T02, the king proximity and blockade adjustments are additive modifiers applied on top.
- `KING_PROXIMITY_OWN_BONUS: i32` — per-distance-unit bonus for own king being close to a passed pawn (endgame-weighted)
- `KING_PROXIMITY_OPP_BONUS: i32` — per-distance-unit bonus for opponent king being far from a passed pawn (endgame-weighted)
- `BLOCKED_PASSED_PAWN_PENALTY_MG: i32` and `BLOCKED_PASSED_PAWN_PENALTY_EG: i32` — penalty when a piece occupies the advance square

**New public function**:
- `pub fn evaluate_passed_pawn_extras(pos: &Position) -> (i32, i32)` — iterates over all passed pawns for both colors (using the same detection logic from `evaluate_color`), and for each passed pawn:
  ...
  4. Awards mg/eg king proximity bonuses (primarily endgame-weighted)
  5. Applies blockade penalty if the advance square is occupied
  6. Returns `(white_extras_mg - black_extras_mg, white_extras_eg - black_extras_eg)`

**Helper function** (private):
- `fn chebyshev_distance(sq1: u32, sq2: u32) -> i32` — computes `max(|file1 - file2|, |rank1 - rank2|)` using raw square indices

**New tests**:
- `passed_pawn_rank6_higher_than_rank4` — already exists from T02 (`passed_pawn_bonus_scales_by_rank`); the advancement bonus is already in `evaluate_color`. No new test needed for this.
- `blocked_passed_pawn_lower_than_unblocked` — White passed pawn on d5 with a piece on d6 vs White passed pawn on d5 with d6 empty; blocked version has lower score.
- `king_proximity_affects_score` — White passed pawn on d5, White king on e5 (close) vs White king on a1 (far); close king yields higher eval. Opponent king similarly tested.
- `pawn_hash_end_to_end` — Call `evaluate()` twice with same position, verify the pawn hash table is populated on first call and returns cached result on second call (indirectly verified by result consistency).
...
- Call `pawn::evaluate_passed_pawn_extras(pos)` to get position-dependent `(pp_extra_mg, pp_extra_eg)` (these are NOT cached because they depend on king positions and all pieces)
...
- At line ~128 (`quiescence`): change `chess_eval::evaluate(pos)` to `chess_eval::evaluate(pos, &mut ctx.pawn_table)`
- At line ~329 (`negamax`): change `chess_eval::evaluate(pos)` to `chess_eval::evaluate(pos, &mut ctx.pawn_table)`
...
1. **`blocked_passed_pawn_lower_than_unblocked`**: FEN with White passed pawn on d5, piece on d6 (blocked) vs FEN with d6 empty. Call `evaluate_passed_pawn_extras` on both positions. Assert blocked version yields lower (more negative) extra score.

2. **`king_proximity_own_king_close_scores_higher`**: FEN with White passed pawn on d5, White king on e5 vs White king on a1. Call `evaluate_passed_pawn_extras`. Assert close-king position has higher extra score.

3. **`king_proximity_opp_king_far_scores_higher`**: FEN with White passed pawn on d5, Black king on h8 (far) vs Black king on e6 (close). Assert far-opponent-king gives higher score.
...
- This requires changing `evaluate()`'s signature from `fn evaluate(pos: &Position) -> i32` to `fn evaluate(pos: &Position, pawn_table: &mut PawnHashTable) -> i32`, which propagates to the 2 production call sites in the search crate and 2 test call sites.

Now I have all the context I need. The feedback is clear: add one missing integration-level test that verifies a passed pawn on rank 6 scores higher than one on rank 4 through the full `evaluate()` pipeline with `PawnHashTable`. Let me produce the complete revised plan.

# T03 — Passed Pawn Evaluation: Implementation Plan (v3)
...
### `crates/eval/src/lib.rs` — New and Updated Tests

4. **`passed_pawn_rank6_scores_higher_than_rank4`** (integration-level): Create two positions — one with a White passed pawn on d6 (rank 6) and one with a White passed pawn on d4 (rank 4), with identical king placements and no other pawns. Create a `PawnHashTable`, call `evaluate(&pos, &mut pawn_table)` on each position. Assert the rank-6 position scores higher than the rank-4 position. This verifies the full pipeline: pawn hash computation → probe/miss → `evaluate_pawns()` → store → tapered interpolation correctly reflects rank-based advancement bonuses.

   Example FENs:
   - Rank 6: `4k3/8/3P4/8/8/8/8/4K3 w - - 0 1`
   - Rank 4: `4k3/8/8/8/3P4/8/8/4K3 w - - 0 1`

5. **`pawn_hash_end_to_end`**: Create a position, create a `PawnHashTable`, call `evaluate()` twice. Assert both calls return the same score (verifying cache consistency).

6. **Existing tests updated**: `startpos_eval_is_zero`, `white_advantage_positive_for_white`, `white_advantage_negative_for_black`, `centralized_knight_scores_higher_than_rim_knight`, `tapered_eval_interpolation`, `pst_bonuses_are_additive_with_material` — all updated to pass `&mut PawnHashTable::new()` as second argument. The `tapered_eval_interpolation` test uses a position with no pawns, so pawn eval contributes 0 and the expected value remains 344.
...
8. **Add `passed_pawn_rank6_scores_higher_than_rank4` integration test** in `crates/eval/src/lib.rs`
9. **Add `pawn_hash_end_to_end` test** in `crates/eval/src/lib.rs`
10. **Add `pawn_table` field** to `SearchContext` in `crates/search/src/lib.rs`
11. **Update `search()` function** to initialize `pawn_table` in `SearchContext`
12. **Update `evaluate()` call sites** in `quiescence` and `negamax` in `crates/search/src/lib.rs`
13. **Update `test_ctx()` and search tests** in `crates/search/src/lib.rs`

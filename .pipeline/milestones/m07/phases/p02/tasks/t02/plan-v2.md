...
### Modified File
- **`crates/eval/src/lib.rs`** — Add `pub mod mobility;` declaration, import and call `mobility::evaluate_mobility()` for both colors, and add the resulting midgame/endgame scores to the accumulators before tapered interpolation.
...
1. **Compute opponent pawn attacks**: Given the opponent's pawn bitboard, compute all squares attacked by opponent pawns using bitwise shifts. For white pawns: `((pawns & NOT_A_FILE) << 7) | ((pawns & NOT_H_FILE) << 9)`. For black pawns: `((pawns & NOT_H_FILE) >> 7) | ((pawns & NOT_A_FILE) >> 9)`. This produces a `Bitboard` of all squares controlled by opponent pawns.

2. **Compute safe squares**: The mobility mask is `!opponent_pawn_attacks` — squares NOT attacked by opponent pawns. Pieces can move to any square in their attack set that intersects this mask. Friendly pieces are NOT excluded from the mobility count (standard convention: pseudo-legal mobility counts all attacks minus pawn-controlled squares).
...
- With pawns: `4k3/8/8/3pp3/8/4N3/8/4K3 w - - 0 1` (knight on e3; black pawns on d5 and e5 attack c4, d4, e4, and f4 — of the knight's eight target squares {c2, c4, d1, d5, f1, f5, g2, g4}, squares c4 and f4 are attacked by opponent pawns and thus excluded from safe mobility; additionally d5 is occupied by a pawn but would still be counted since friendly-piece exclusion doesn't apply — however d5 is an opponent pawn so it is a valid attack target, though if it's also attacked by the d5-pawn's neighbor that is already counted)
...
**Setup**: Two positions where a knight and a queen each have exactly 8 pseudo-legal safe moves.
- Knight with 8 moves: `4k3/8/8/8/4N3/8/8/4K3 w - - 0 1` (knight on e4 has 8 target squares: c3, c5, d2, d6, f2, f6, g3, g5 — all safe with no opponent pawns)
- Queen with 8 moves: `4k3/8/8/8/2q5/1Q6/PP6/K7 w - - 0 1` is too constrained; instead use a position where queen mobility can be directly verified.
- More precisely: the test does not need equal move counts. With knight on e4 having 8 moves, the knight MG contribution is `8 * 4 = 32`. A queen needs more than 32 moves at `1` per move (i.e., more than 32 safe squares) for its total to exceed the knight's. This means any position where a queen has fewer than 33 safe moves will score less than a centralized knight with 8 moves. Use:
  - Knight position: `4k3/8/8/8/4N3/8/8/4K3 w - - 0 1` — knight on e4, 8 moves, MG = 32
  - Queen position: `4k3/8/8/8/4Q3/8/8/4K3 w - - 0 1` — queen on e4, 25 safe moves (no opponent pawns, no blocking pieces aside from own king on e1), MG = `25 * 1 = 25`
**Verify**: Knight position MG score (32) > queen position MG score (25), demonstrating that queen per-move scaling is lighter than knight per-move scaling.

### Test 5: `mobility_produces_midgame_and_endgame_scores`
**Setup**: Any non-trivial position with pieces.
**Verify**: Both MG and EG components of the return tuple are > 0 when pieces have available moves.

### Integration test in `lib.rs` (existing test `startpos_eval_is_zero`):
The existing `startpos_eval_is_zero` test continues to pass since mobility is symmetric in the starting position and the net contribution is zero.
...
2. **Modify `crates/eval/src/lib.rs`**: Add `pub mod mobility;` declaration and integrate the mobility call into `evaluate()` between the king safety block and the tapered interpolation.
...
- **Parent spec requires**: "count pseudo-legal moves per piece (excluding squares attacked by opponent pawns)"

Now I have all the information needed. Let me compute the exact new expected value for the `tapered_eval_interpolation` test.

For the position `4k3/8/8/8/3N4/8/8/4K3 w - - 0 1`:
- White knight on d4 (square index 27), white king on e1 (square index 4), black king on e8 (square index 60)
- Phase: 1 knight = 1 (from compute_phase)

Current score (346) already accounts for material + PST + king safety. I need to compute the mobility addition.

Knight on d4 (sq 27): attacks from KNIGHT_ATTACKS[27]. d4 = rank 4, file d. Knight moves: c2, e2, b3, f3, b5, f5, c6, e6 — that's 8 squares. No opponent pawns, so all 8 are safe.

White mobility: 8 moves × 4 MG = 32 MG, 8 × 4 EG = 32 EG.
Black mobility: no pieces (only king), so 0 MG, 0 EG.

Net mobility: mg_score += 32, eg_score += 32.

Previous score without mobility:
- Let me reverse-engineer from the current value of 346 and phase=1.
- score = (mg * 1 + eg * 23) / 24 = 346
- So mg + 23*eg = 346 * 24 = 8304

With mobility added: mg' = mg + 32, eg' = eg + 32
- new_score = ((mg+32)*1 + (eg+32)*23) / 24 = (mg + 23*eg + 32 + 32*23) / 24 = (8304 + 32 + 736) / 24 = 9072 / 24 = 378

The new expected value is **378**.

Now let me also check king safety for this position to make sure I understand correctly. Both kings have no pawns (open files), so they get penalties. This was already integrated in T01.

Now I have everything needed. Let me produce the complete revised plan.

# T02 — Mobility Evaluation: Implementation Plan (v3)
...
- With pawns: `4k3/8/8/3pp3/8/4N3/8/4K3 w - - 0 1` (knight on e3; black pawns on d5 and e5 attack c4 and f4 respectively — these squares are excluded from safe mobility)
...
- Knight position: `4k3/8/8/8/4N3/8/8/4K3 w - - 0 1` — knight on e4, 8 safe moves, MG = 8 × 4 = 32
- Queen position: `4k3/8/8/8/4Q3/8/8/4K3 w - - 0 1` — queen on e4, 25 safe moves (no opponent pawns, only own king on e1 in the way on one ray), MG = 25 × 1 = 25

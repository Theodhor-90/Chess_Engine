...
- `4k3/8/8/3pp3/8/4N3/8/4K3 w - - 0 1` (knight on e3; d5 and f4 attacked by black pawns)
- `4k3/8/8/8/8/4N3/8/4K3 w - - 0 1` (knight on e3; no opponent pawns)
**Verify**: Mobility count with opponent pawns is less than without.

### Test 4: `queen_mobility_scaled_less_than_knight`
**Setup**: Position where a queen and a knight have similar move counts, verify queen's per-move contribution is lower.
- Position with a lone knight having N moves and a lone queen having N moves.
**Verify**: Queen total MG score < Knight total MG score when move counts are equal (or verify via known position where queen has more moves but lower total bonus than a knight with fewer moves).

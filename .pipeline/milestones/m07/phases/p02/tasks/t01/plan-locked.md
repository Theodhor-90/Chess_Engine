...
1. **Correctness issue**: The `?` operator in `pos.piece_bitboard(...).into_iter().next()?` is incompatible with the `(i32, i32)` return type.
2. **Ambiguity issue**: Test 4 needs a concrete assertion and specific FEN.
...
If no king is found (which should not occur in a legal position but must be handled to satisfy the `(i32, i32)` return type without panicking), the function returns `(0, 0)`. This avoids the `?` operator (which requires `Option` return type) and complies with the project convention of no `unwrap()` in library crates.
...
- Strong: `"4k3/8/8/8/8/8/5PPP/6K1 w - - 0 1"` (intact shield on f2/g2/h2 for king on g1)
...
- Pressure: `"4k3/8/8/8/8/8/5PPP/4r1K1 w - - 0 1"` — Black rook on e1 near White king on g1.
...
**Test 4: Midgame shield bonus is larger than endgame shield bonus**
- FEN: `"4k3/8/8/8/8/8/5PPP/6K1 w - - 0 1"` (White king on g1 with intact f2/g2/h2 shield).
- Call `evaluate_king_safety(pos, Color::White)` and capture `(mg, eg)`.
- Assert: `assert!(mg > eg, "midgame shield bonus {mg} should exceed endgame shield bonus {eg}")`.
- Rationale: The pawn shield constants are `PAWN_SHIELD_BONUS_MG: [35, 20, 8, 0]` vs `PAWN_SHIELD_BONUS_EG: [10, 5, 2, 0]`, so with three intact shield pawns at rank distance 1, the midgame contribution (3 × 35 = 105) will exceed the endgame contribution (3 × 10 = 30). The attack scaling also weights midgame higher (`ATTACK_SCORE_SCALE_MG = 4` vs `ATTACK_SCORE_SCALE_EG = 1`), but with no opponent pieces in this position the attack component is zero, leaving only the shield and open-file terms. Since there are no open files either, the totals are purely from shield bonuses: `mg = 105 > eg = 30`.
...
// Returns (0, 0) if no king is found for the given color.
```
...
| `midgame_shield_bonus_exceeds_endgame` | Intact shield position produces `mg > eg` because `PAWN_SHIELD_BONUS_MG` values (35/20/8/0) exceed `PAWN_SHIELD_BONUS_EG` values (10/5/2/0) |

...
- Value: `64` (centipawns)
- Placed alongside existing constants like `FUTILITY_MARGINS`, `REVERSE_FUTILITY_MARGINS`, and `IID_MIN_DEPTH` near the top of `lib.rs`
...
1. **Add constants**: Add `SINGULAR_MARGIN` and `SINGULAR_MIN_DEPTH` constants near the top of `lib.rs`, alongside existing constants like `MATE_SCORE`, `INFINITY`, `IID_MIN_DEPTH`, `MAX_PLY`, `FUTILITY_MARGINS`, and `REVERSE_FUTILITY_MARGINS`.

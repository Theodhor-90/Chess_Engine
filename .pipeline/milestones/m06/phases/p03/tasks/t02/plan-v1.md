Good — there is currently no exclusion mechanism. The singular extension verification search needs to exclude the TT move. Let me now check how the `negamax` function signature is called to understand if adding a parameter for an excluded move or using a different approach would be more appropriate.

Now I have all the information needed to produce the implementation plan. Let me review what the task spec requires:

1. Singular extension logic in `negamax` in `crates/search/src/lib.rs`
2. Singular extension margin constant (initial value: 64 centipawns)
3. Unit tests for activation and non-activation conditions
4. Only attempt at depth ≥ 6
5. TT move must exist with sufficient depth
6. Verification search at `depth / 2`, excluding TT move, zero-window at `tt_score - margin`
7. Extend if all alternatives score below threshold
...
- Value: `64` (centipawns)
- Placed alongside the existing constants (`FUTILITY_MARGINS`, `REVERSE_FUTILITY_MARGINS`, `IID_MIN_DEPTH`, etc.) near the top of `lib.rs`
...
- **Setup**: Use a quiet position (e.g., starting position) where multiple first moves are roughly equal. Run iterative deepening to depth 8. Compare nodes with `singular_extension_enabled: true` vs `false`.
- **Assertion**: Node counts should be very similar (or identical at lower depths), because no single TT move dominates by the margin — the verification search finds alternatives scoring within range, so the extension does not trigger.
...
1. **Add constants**: Add `SINGULAR_MARGIN` and `SINGULAR_MIN_DEPTH` constants near the top of `lib.rs`, alongside existing constants.

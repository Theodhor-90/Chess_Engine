Now I have all the context. The feedback is clear: the only issue is that line 1 contains preamble LLM reasoning text that must be removed. The plan should begin directly with `## 1. Deliverables`. The plan content itself is solid and implementation-ready.
...
3. **SearchContext initialization**: In the `search` function, initialize `killers: killer::KillerTable::new()` in the `SearchContext` constructor. Clear killers at the start of each iterative deepening iteration is not needed since the same killer table accumulates useful data across iterations (standard practice).
...
6. **quiescence — no change to ordering call**: Quiescence search only searches captures/promotions, so killer moves (which are quiet) do not apply. However, the `order_moves` signature changed, so the call in `quiescence` needs to be updated. Since quiescence only has tactical moves, killers won't match any of them. Two options: (a) pass `&ctx.killers` and `ply` anyway (harmless, killers won't match captures), or (b) keep using the old scoring path. Option (a) is simpler — pass the arguments through and the killer check will simply never match on capture moves.

7. **search — clear killers**: Call `ctx.killers.clear()` at the start of `search` (already initialized fresh via `KillerTable::new()`). No need for an explicit clear since the struct is newly created.

8. **test_ctx helper**: Update `test_ctx()` in tests to include the `killers` field.
...
- Action: Store A, store B, store A again.
- Assert: `is_killer(ply, A)` is `true` and `is_killer(ply, B)` is `true` (B was not evicted because A was already in slot 0 and the store was a no-op).

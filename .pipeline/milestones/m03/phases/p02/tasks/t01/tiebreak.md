Now I have all the information. Here is the definitive plan:
...
|------|-------|-----------|
| `checkmate_black_is_mated` | FEN: `"rnb1kbnr/pppp1ppp/8/4p3/6Pq/5P2/PPPPP2P/RNBQKBNR w KQkq - 1 3"` (Scholar's mate, White to move, White is mated — no legal moves, king in check) | Returns `(-MATE_SCORE + ply, None)` where ply is 0 at root. Score is `-30000`. |
...
| `stalemate_returns_zero` | FEN: `"k7/1R6/K7/8/8/8/8/8 b - - 0 1"` — Black king on a8, White rook on b7, White king on a6. Black has no legal moves: Ka7 attacked by both Rb7 (rank 7) and Ka6 (adjacent); Kb8 attacked by Rb7 (b-file); Kxb7 illegal (king moves into check from Ka6). Black king is not in check. | Returns `(0, None)`. |

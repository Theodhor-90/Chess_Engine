I now have all the context needed to write the phase specification. Let me produce it.
...
3. **Mate and stalemate scoring**: Use a `MATE_SCORE` constant (e.g., 30000) adjusted by ply distance (`-MATE_SCORE + ply` for being mated, `MATE_SCORE - ply` for delivering mate) so the search prefers shorter mates. Stalemate returns 0 (draw).
...
5. **Time management via `Instant`**: Use `std::time::Instant` to track elapsed time. The search checks elapsed time every N nodes (e.g., 1024) and aborts cleanly if the time budget is exceeded, returning the best move from the last fully completed iteration.
...
Implement iterative deepening that searches at depth 1, 2, 3, etc., collecting the best move at each completed depth. Add basic time management: accept a fixed time budget, check elapsed time periodically during search (every N nodes), and abort cleanly when time is exhausted. Return the best move from the last fully completed depth. Expose a top-level search entry point that takes a `Position` and time budget.

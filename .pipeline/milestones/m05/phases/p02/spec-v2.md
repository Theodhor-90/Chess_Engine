Now I have all the context. The feedback has exactly two issues, both about IID depth threshold ambiguity. Let me produce the revised specification.
...
2. **IID triggers at depth >= 4 when no TT move is available** — IID performs a reduced-depth search (depth reduced by 2) at nodes where the remaining depth is at least 4 and no TT move was found on probe. The threshold of 4 is a compile-time constant `const IID_MIN_DEPTH: i32 = 4;` defined in `crates/search/src/lib.rs`. The reduced search populates the TT, so the subsequent full-depth search benefits from TT move ordering. IID reuses the existing `negamax` function — no separate search routine is needed.
...
Implement IID in the `negamax` function. At nodes where the remaining depth is at least 4 (`IID_MIN_DEPTH`) and no TT move was found on probe, perform a reduced-depth search (depth - 2) to populate the TT, then re-probe the TT to obtain a move for ordering. The threshold is a compile-time constant `const IID_MIN_DEPTH: i32 = 4;` defined at the top of `crates/search/src/lib.rs`. Add tests that confirm: (a) IID reduces total node count compared to search without IID on positions at sufficient depth, (b) IID does not trigger at depths below 4, (c) IID correctly finds a move that the subsequent full-depth search uses for ordering.

**Deliverables:**
- Compile-time constant `const IID_MIN_DEPTH: i32 = 4;` in `crates/search/src/lib.rs`
- IID logic in `negamax` in `crates/search/src/lib.rs` that triggers when `depth >= IID_MIN_DEPTH` and no TT move is available
...
- `crates/search/src/lib.rs` — Modified: `IID_MIN_DEPTH` constant, IID logic in `negamax`, position history stack in `SearchContext`, repetition and fifty-move rule detection, updated `search` function signature, TT move legality validation

...
**Setup:**
- Use a complex middlegame position (e.g., `"r1bqkb1r/pppppppp/2n2n2/8/4P3/5N2/PPPP1PPP/RNBQKB1R w KQkq - 2 3"`).
...
- Clear both TTs (no prior entries, so no TT move is available).
- Call `negamax` directly at `depth = 3` (below threshold) on both contexts.
- Compare node counts — they should be identical because IID cannot trigger at depth 3.
- Additionally, call `negamax` at `depth = 4` (at threshold) and verify that it does search more nodes than a single `negamax` at depth 4 with an already-populated TT (indicating IID ran the extra reduced search).

**Assertion for depth 3:** Both contexts produce the same node count (IID inactive).

**Assertion for depth 4:** A search at depth 4 on a fresh TT uses more nodes than a search at depth 4 on a pre-populated TT (IID runs and adds overhead at depth 4 on the fresh TT, but the full-depth search benefits from better ordering). Or more simply: the fresh-TT search at depth 4 produces a valid move (no crash), confirming the code path works at the threshold.

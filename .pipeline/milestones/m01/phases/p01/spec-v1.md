Now I have all three context files. Let me produce the phase specification.
...
6. **Iterator-based bit traversal.** `Bitboard` implements `Iterator` (or provides an iterator via a method) that yields `Square` values for each set bit, using `trailing_zeros` to extract the LSB. This provides idiomatic Rust iteration over piece locations.
...
**Deliverables:**
- `Bitboard` struct with `BitAnd`, `BitOr`, `BitXor`, `Not`, `Shl`, `Shr` operator overloads (including assign variants `BitAndAssign`, `BitOrAssign`, `BitXorAssign`)
...
- Constants for common bitboards (e.g., `EMPTY`, `ALL`)
- Unit tests covering operator behavior, population count, LSB/MSB extraction, and iteration over various bit patterns including edge cases (empty bitboard, single bit, all bits set)

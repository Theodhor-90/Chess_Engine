...
**Display formatting:**
- Implement `fmt::Display` for `Bitboard` — renders an 8×8 grid with `1` for set bits and `.` for unset bits, rank 8 at the top and rank 1 at the bottom (standard board orientation). This provides a human-readable board visualization for debugging purposes, complementing the derived `Debug` which provides the programmatic struct representation needed by `assert_eq!` and other macros.

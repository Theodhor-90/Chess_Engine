Now I have all the context. The feedback identifies one issue: the en passant clearing sequence is ambiguous in the algorithm steps. The recommendation is to clear en passant immediately after saving undo state (moving step 11 to between steps 2 and 3), then only set it in the double pawn push branch. Let me revise the plan accordingly.
...
1. **Save undo state**: Capture current `castling_rights`, `en_passant`, `halfmove_clock`, and `hash` into `UndoInfo`. The `captured` field is set during move execution (initialized to `None`).

2. **Extract move info**: `from = mv.from_sq()`, `to = mv.to_sq()`, `flags = mv.flags()`. Determine the moving piece via `self.piece_on(from)` (will be `Some` for valid moves).

3. **Clear en passant unconditionally**: If `self.en_passant` is `Some`, XOR out its file key from the Zobrist hash, then set `self.en_passant = None`. This is done immediately after saving undo state so that en passant is always cleared before any move-type-specific logic runs. Only the double pawn push branch (step 6) will re-set it.

4. **Hash out old castling rights**: XOR out `castling_key(self.castling_rights)`.

5. **Determine captured piece**: For regular captures and promotion-captures, read `self.piece_on(to)` and store in `UndoInfo.captured`. For en passant, the captured pawn is on a different square (computed in step 6).

6. **Handle by move type** (branching on `flags`):
   ...
   **Double pawn push** (`DOUBLE_PAWN_PUSH`):
   ...
   - This is the **only** branch that sets `self.en_passant` (it was unconditionally cleared in step 3)
   ...
   **En passant** (`EN_PASSANT`):
   ...
   - Remove captured pawn from that square (set `UndoInfo.captured` to the opponent pawn)
...
7. **Update castling rights**: Apply the mask table for both `from` and `to` squares: `self.castling_rights.mask(CASTLING_RIGHTS_MASK[from.index() as usize])` and same for `to`. This handles king moves, rook moves, and rook captures in a single branchless operation.

8. **Hash in new castling rights**: XOR in `castling_key(self.castling_rights)`.

9. **Update halfmove clock**: Reset to 0 if moving piece is a pawn or if it's a capture; otherwise increment by 1.

10. **Update fullmove counter**: Increment by 1 if side to move was Black.

11. **Toggle side to move**: Flip to opposite color. Hash: XOR in `side_to_move_key()`.
...
| `make_move_clears_prior_ep` | When a non-double-pawn-push move is made from a position that has an en passant square, the en passant square is cleared and the Zobrist hash correctly XORs out the old EP file key without XORing in a new one. |

Now I have all the information needed. Let me produce the complete revised plan.
...
Add a `pub(crate) fn new(...)` constructor to `impl Position`. This allows `fen.rs` to construct `Position` instances without exposing fields publicly:
...
2. **Piece placement (field 0)**: Split field by `'/'`. Validate exactly 8 rank strings; if count ≠ 8, return `FenError::InvalidPiecePlacement("expected 8 ranks")`. Initialize `let mut piece_bb = [Bitboard::EMPTY; 12]`. Iterate ranks in reverse (FEN lists rank 8 first, rank 1 last), using `enumerate()` on the split. For each `(rank_str_idx, rank_str)`, compute `rank_idx = 7 - rank_str_idx` and convert to enum: `let rank = Rank::try_from(rank_idx as u8).map_err(|_| FenError::InvalidPiecePlacement(...))`. Track `file_idx: u8 = 0`. For each character `ch` in `rank_str`:
   - If `ch` is a digit `'1'`–`'8'`: advance `file_idx` by `ch as u8 - b'0'`.
   - If `ch` is a piece character: map to `Piece` using the explicit match table below, compute the file via `let file = File::try_from(file_idx).map_err(|_| FenError::InvalidPiecePlacement(...))`, compute the square via `let sq = Square::from_file_rank(file, rank)`, set the bit `piece_bb[piece.index()] = piece_bb[piece.index()] | Bitboard::new(1u64 << sq.index())`. Advance `file_idx` by 1.
...
4. **Castling availability (field 2)**: If the field is `"-"`, set `let castling_rights = CastlingRights::NONE`. Otherwise, start with `let mut castling_rights = CastlingRights::NONE` and iterate characters. For each character:
   - `'K'` → `castling_rights.set(CastlingRights::WHITE_KINGSIDE)`
   - `'Q'` → `castling_rights.set(CastlingRights::WHITE_QUEENSIDE)`
   - `'k'` → `castling_rights.set(CastlingRights::BLACK_KINGSIDE)`
   - `'q'` → `castling_rights.set(CastlingRights::BLACK_QUEENSIDE)`
   - Any other character → return `FenError::InvalidCastling(field.to_string())`

   Also validate no duplicate characters: if any flag is already set when encountered, return `FenError::InvalidCastling`. Check via `castling_rights.contains(flag)` before calling `set`.
...
5. **En passant (field 3)**: If the field is `"-"`, set `let en_passant = None`. Otherwise:
   ...
   - Extract characters: `let mut chars = field.chars(); let file_char = chars.next().unwrap(); let rank_char = chars.next().unwrap();`
   - Convert file: `let file_idx = file_char as u8 - b'a'`. If `file_char` is not in `'a'..='h'`, this will produce an out-of-range value. Convert via `let file = File::try_from(file_idx).map_err(|_| FenError::InvalidEnPassant(field.to_string()))?`.
   - Convert rank: `let rank_idx = rank_char as u8 - b'1'`. If `rank_char` is not in `'1'..='8'`, this will produce an out-of-range value. Convert via `let rank = Rank::try_from(rank_idx).map_err(|_| FenError::InvalidEnPassant(field.to_string()))?`.
...
1. **Piece placement**: Iterate rank indices from 7 down to 0 (rank 8 to rank 1). For each `rank_idx`, convert via `Rank::try_from(rank_idx as u8).unwrap()` (safe because `rank_idx` is always 0..7). Iterate file indices 0..8. For each `file_idx`, convert via `File::try_from(file_idx as u8).unwrap()`. Compute `let sq = Square::from_file_rank(file, rank)`. Call `pos.piece_on(sq)`:
   - If `Some(piece)`: flush any accumulated empty count by appending the digit character, then append the piece character using the `Piece` `Display` impl (via `write!(result, "{piece}")` which produces `KQRBNPkqrbnp`). Reset empty counter to 0.
   - If `None`: increment empty counter.
   
   After processing all 8 files in a rank, flush any remaining empty count. If `rank_idx > 0`, append `'/'`.

2. **Active color**: Append `" w"` if `pos.side_to_move() == Color::White`, `" b"` if `Color::Black`.

3. **Castling availability**: Append a space. Check each flag in order: if `pos.castling_rights().contains(CastlingRights::WHITE_KINGSIDE)` append `'K'`, if `WHITE_QUEENSIDE` append `'Q'`, if `BLACK_KINGSIDE` append `'k'`, if `BLACK_QUEENSIDE` append `'q'`. If no flags were set, append `'-'`.

4. **En passant**: Append a space. If `pos.en_passant()` is `Some(sq)`, use `Square`'s `Display` impl to format (produces e.g. `"e3"`). If `None`, append `'-'`.

5. **Halfmove clock**: Append a space, then `pos.halfmove_clock()` as decimal.

6. **Fullmove number**: Append a space, then `pos.fullmove_counter()` as decimal.
...
- Parse starting FEN, compare all piece bitboards with `Position::startpos()` to verify they agree on every field (all 12 piece bitboards, occupancy, side to move, castling rights, en passant, halfmove clock, fullmove counter)

**T5: `reject_wrong_field_count`**
- FEN with 5 fields (missing fullmove): assert `Err` and match against `FenError::WrongFieldCount(5)`

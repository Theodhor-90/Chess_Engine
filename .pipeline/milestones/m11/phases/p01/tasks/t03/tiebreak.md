...
- **No new external dependencies**: Changelog generation uses git commands available in the GitHub Actions runner. The `softprops/action-gh-release@v2` action already supports a `body_path` parameter for the release body.
...
Add a new step in the `package` job, after the "Checkout" step and before the "Download all artifacts" step. This step:
